//Version 0.5.4 //Tag Implementation
//REQUIRE DE-Module. 
de.provider('DeGalleryControl', function() {
	...
	this.$get=function() {
		...
		this.toggleGallery=function(open) {
			...
			if (this.showGallery==true && this.galleryLoaded==false) {
		...
		this.setGalleryScope=function(galleryScope) {
	...
	return {
		...
		controller:function($scope, $element, DeMultimedia, DeGalleryControl) {
			...
			$scope.deleteFile=function(index) {
				...
				file.removing=true;
				DeMultimedia.remove($scope.gid, file.id).then(function(res) {
...
var galleryTemplate='<div class="gallery-mask" ng-show="shouldShow()" ng-click="closeGallery()"></div><div class="gallery-container" ng-init="initGallery()" ng-attr-id="gallery-{{id}}" ng-show="shouldShow()"><header class="gallery-header"><h2>Choose an Image</h2><button type="button" class="btn-op btn-close" ng-click="closeGallery()"></button></header><nav class="gallery-nav nav-tabs"><button type="button" class="btn-tab" ng-click="activateTab(\'imgTab\')" ng-class="{active : imgTabActive}">Gallery</button><button type="button" class="btn-tab" ng-click="activateTab(\'fileTab\')" ng-class="{active : fileTabActive}">Upload</button></nav><section class="gallery-content"><div class="gallery-tab img-tab" ng-show="imgTabActive"><div class="gallery-img-content"><div class="gallery-img-container" ng-repeat="imgObj in imageData"><button type="button" class="btn-op btn-remove" ng-click="deleteFile($index)"></button><div class="gallery-img" ng-click="previewImg(imgObj)"><img ng-src="{{imgObj.sizes.thumb.path}}" ng-if="imgObj.type==\'image\'"><i class="material-icons" ng-if="imgObj.type!=\'image\'">insert_drive_file</i><span ng-bind="imgObj.name" ng-if="imgObj.type!=\'image\'"></span></div><div class="img-remove" ng-if="imgObj.removing"></div></div></div><div class="gallery-preview-pane" ng-show="selectedImg"><div class="gallery-img-container"><div class="gallery-img"><img ng-src="{{selectedImg.sizes.medium.path}}" ng-if="selectedImg.type==\'image\'"><i class="material-icons" ng-if="imgObj.type!=\'image\'">insert_drive_file</i><span ng-bind="selectedImg.name" ng-if="selectedImg.type!=\'image\'"></span></div></div><div class="gallery-img-meta"><dl><dt>Image Name: </dt><dd><span ng-bind="selectedImg.name"></span></dd><dt>Extension: </dt><dd><span ng-bind="selectedImg.extension"></span></dd><dt>Orientation:</dt><dd><span ng-bind="selectedImg.orientation"></span></dd><dt>Path: </dt><dd><span ng-bind="selectedImg.path"></span></dd><dt>Alt Text:</dt><dd><input type="text" ng-model="selectedImg.alt" placeholder="Alt text"></dd><dt>Photographer:</dt><dd><input type="text" ng-model="selectedImg.author" placeholder="Photographer"></dd><dt>Credit Link:</dt><dd><input type="text" ng-model="selectedImg.credit_url" placeholder="Image credit\'s URL. "></dd></dl></div><div class="gallery-img-action"><button type="button" class="btn-select" ng-click="selectImage()">Select Image</button></div></div></div><div class="gallery-tab file-tab" ng-show="fileTabActive"><div class="gallery-file-content"><div class="gallery-file-dropzone"></div><div class="gallery-file-container" ng-repeat="(fileKey,fileObj) in fileQueue"><div class="gallery-img-container"><button type="button" class="btn-op btn-remove" ng-click="removeFile(fileKey)"></button><div class="gallery-img"><div de-preview-file="fileObj.file"></div><div class="gallery-file-progress" ng-if="fileObj.isloading"><span>Uploading</span></div></div></div><label class="file-name" ng-bind="fileObj.file.name"></label><span class="file-size" ng-bind="fileObj.file.size | bytes"></span></div></div><div class="gallery-file-action"><input type="file" id="upload-file" de-bind-files="addFile(file)" multiple><button type="button" ng-click="uploadFiles()">Upload All</button></div></div></section></div>';
